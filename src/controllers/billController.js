...
const { createInvoicePdf, money } = require("../utils/premiumPdf");
...
const downloadBill = asyncHandler(async (req, res) => {
  ...
  const pdf = createInvoicePdf({
    ...
    invoiceId: bill.invoiceId || bill._id,
    residentName: bill.tenant ? bill.tenant.name : "",
    flatNo: bill.flatNo,
    meterNumber: bill.tenant ? bill.tenant.meterNumber || "Not set" : "Not set",
    month: bill.month,
    total,
    status: bill.status,
    rows: [
      ["Monthly rent", money(bill.rent)],
      ["Electricity", money(bill.electricity)],
      ["Water", money(bill.water)],
      ["Gas", money(bill.gas)],
      ["Service charge", money(bill.serviceCharge)],
      ["Other costs", money(bill.otherCosts)]
    ]
  });
